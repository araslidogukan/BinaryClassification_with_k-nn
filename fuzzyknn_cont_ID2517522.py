...
def my_cont_fuzzyknn(train_data_x,train_data_y,test_data_x,test_data_y,k,m):
    ...
    class_center_1 = np.mean(train_data_x[train_data_y == 1,:],axis=0).reshape([1,-1])
    class_center_0 = np.mean(train_data_x[train_data_y == 0,:],axis=0).reshape([1,-1])

    train_class_memberships_1 = np.linalg.norm(train_data_x - class_center_0,axis=1) / (np.linalg.norm(train_data_x - class_center_0,axis=1) + np.linalg.norm(train_data_x - class_center_1,axis=1))
    train_class_memberships_0 = np.linalg.norm(train_data_x - class_center_1,axis=1) / (np.linalg.norm(train_data_x - class_center_0,axis=1) + np.linalg.norm(train_data_x - class_center_1,axis=1))
    ...
    k_labels_train_1 = train_class_memberships_1[knn_indices_train]
    k_labels_train_0 = train_class_memberships_0[knn_indices_train]
    k_labels_test_1 = train_class_memberships_1[knn_indices_test]
    k_labels_test_0 = train_class_memberships_0[knn_indices_test]
    ...
    train_label1_score = np.sum((k_labels_train_1*(1/weights_train**(2/(m-1)))),axis=1) / np.sum(1/weights_train**(2/(m-1)),axis=1)
    train_label0_score = np.sum((k_labels_train_0*(1/weights_train**(2/(m-1)))),axis=1) / np.sum(1/weights_train**(2/(m-1)),axis=1)

    test_label1_score = np.sum((k_labels_test_1*(1/weights_test**(2/(m-1)))),axis=1) / np.sum(1/weights_test**(2/(m-1)),axis=1)
    test_label0_score = np.sum((k_labels_test_0*(1/weights_test**(2/(m-1)))),axis=1) / np.sum(1/weights_test**(2/(m-1)),axis=1)
