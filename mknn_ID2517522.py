...
def my_mknn(train_data_x,train_data_y,test_data_x,test_data_y,k):
    ...
    weights_train[:,1:] = (weights_train[:,-1].reshape([-1,1]) - weights_train[:,1:]) / (weights_train[:,-1] - weights_train[:,0]).reshape([-1,1])
    weights_train[:,0] = 1

    weights_test[:,1:] = (weights_test[:,-1].reshape([-1,1]) - weights_test[:,1:]) / (weights_test[:,-1] - weights_test[:,0]).reshape([-1,1])
    weights_test[:,0] = 1

    train_label1_score = np.sum(weights_train * (k_labels_train == 1),axis = 1)
    train_label0_score = np.sum(weights_train * (k_labels_train == 0),axis = 1)

    test_label1_score = np.sum(weights_test * (k_labels_test == 1),axis = 1)
    test_label0_score = np.sum(weights_test * (k_labels_test == 0),axis = 1)
